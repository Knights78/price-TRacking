...
export async function addProduct(formData){
  ...
  try {
    ...
    //it means productData is there 
    //now i need to check whether already this prpduct user has already searched or not so i willl hceck it in the database 
    ...
    const { data: existingProduct } = await supabase
     ...
     const isUpdate = existingProduct ? true : false;
    ...
    console.log("ERROR in adding the produft",error)
  }
}

export async function deleteProduct(productId)
{
  //i will take product id and delete it from the database 
   try {
    const supabase=await createClient()
   ...
   return {success:true,message:"Product deleted success"}
    ...
    console.log("EROOR in the deleting product API",error)
   }
}
//api to view get the product 
